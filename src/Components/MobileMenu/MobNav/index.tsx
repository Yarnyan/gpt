...
function MobNav({setMenu}: {setMenu: any}) {
            ...
            <Scrollspy onUpdate={(active) => {
            ...
            }} offset={0} className={styles.content} items={pages} currentClassName={styles.active} rootEl='.content' componentTag={'div'}>
                {
                    Tabs.map(({ id, title, icon, to }: TTabs) => {
                        return (
                            <a onClick={() => setMenu(false)} href={"#" + (to !== '/' ? to.substring(1) : 'main')} key={id} className={`${styles.tab} ${id === 1 ? styles.first : ''}`} style={{ gridArea: to !== '/' ? to.substring(1) : 'main' }}>
